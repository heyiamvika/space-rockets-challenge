...
// Package for getting timezone based on GPS coordinates
// Read more: https://github.com/mapbox/timespace
...
export default function Launch() {
	...
	// Getting launchPad details to get it's location
	const { data: launchPad, error: launchPadError } = useSpaceX(
		launch && launch.launch_site
			? `/launchpads/${launch.launch_site.site_id}`
			: null,
	);

	if (error || launchPadError) return <Error />;
	if (!launch || !launchPad) {
		return (
...
function TimeAndLocation({ launch, launchPad }) {
	const { location } = launchPad;
	// GPS coordinates of location
	const point = [location.longitude, location.latitude];
	// Package returns moment-timezone object,
	// from which we can get the timezone name
	const { _z: zone } = ts.getFuzzyLocalTimeFromPoint(
		...
		<SimpleGrid columns={[1, 1, 2]} borderWidth='1px' p='4' borderRadius='md'>
			<Stat>
				...
				<StatNumber fontSize={["md", "xl"]}>
					{formatDateTimeWithTimezone(launch.launch_date_local, zone.name)}
				</StatNumber>
